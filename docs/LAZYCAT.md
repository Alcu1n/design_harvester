...
2. 在懒猫客户端选择本地安装 `release/design-harvester-0.1.6-amd64.lpk`。安装包声明联网与 Compose override 权限，入口由懒猫身份网关保护，仅管理员可见。
...
只要 DESIGN.md 已生成即完成任务；下游 iOS、中文介绍、语言检查与评分继续执行，各阶段模型异常记录为建议。取消／人工标记仍阻止迟到发布。官方校验器及依赖已删除。五维模型评分取等权平均，失败时保存明确标注的保守完整度参考分（0–48），而非伪造模型审核成功或满分。缺失历史分数在查看详情时补齐，不重新采集、不调用模型、不覆盖旧文件。

本机验证：类型检查、18 项单元测试、17 项独立数据库集成测试和生产构建通过。桌面／手机受控页面验证低分完成、完成但有检查建议、规则参考分、报告与手动状态；真实 HTTP 验证历史缺失评分补齐、数据库评分报告下载、READY／FAILED 人工状态持久化。历史补分仅写数据库，兼容 web 的只读 library 挂载；历史原始审核文件保持不变。未进行真实模型语言质量或 NAS 实机验收。

最终 amd64 镜像在独立容器组中通过启动检查：worker 报告 ready，设置／资料库接口返回 200；web 的 library 挂载保持只读，历史补分、数据库报告下载和人工 READY／FAILED 状态均通过真实 HTTP 验证（测试容器显式设置 APP_ORIGIN）。四个核心文件的镜像内 SHA-256 与当前源码一致。

最终包 `release/design-harvester-0.1.5-amd64.lpk`：565,688,320 字节（539.48 MiB），比 0.1.4 小 1.66 MiB。SHA-256：`a8d1faeadc0934f9ce5c748aaab661d6a6b513fbac47b2ba425966e7ed7f2237`。五个 Linux amd64 镜像全量内嵌，32 个唯一层的压缩摘要、解压摘要和层大小全部通过构建脚本校验。未安装 NAS。

## 0.1.6 图片设计分析与简化生成

正常生成链路改为三次模型请求：英文分析与中文 Design DNA 同次生成、英文 iOS 适配、独立质量评分。取消语言／翻译独立审核和检查建议大面板。Overview 聚焦可复用的视觉规则，遵循官方八章结构但不启用格式校验。文档存在即完成的 0.1.5 规则继续生效；技术错误最多额外重试两次，取消、人工状态和授权边界保留。

新增图片来源，可上传同一设计的 1–10 张 PNG/JPEG/WebP，支持排序、删除、拖入、粘贴和移动端文件选择。原图、预览、模型输入和证据分别保存；图片任务不调用浏览器，不冒充 DOM 实测值，单图的跨页面一致性不计入评分。数据库新增 source_kind、import_id/source_metadata 和 presentation_state 等兼容字段；旧版本与用户整理保持原样。

web 仅对 `/data/imports` 有上传写权限，正式 `/data/library` 继续只读。worker 归档完成后清理暂存；未关联暂存到期清理。旧中文缓存只有与当前英文候选一致才恢复；缺失介绍可以单独生成，不改变原始任务、文档、评分或默认版本。

本机验证：类型检查、20 项单元测试、24 项独立数据库集成测试及生产构建通过。受控模型测试覆盖三次调用、技术重试、缓存恢复、中文单独补生成、取消、旧候选不串用、历史／默认版本保护，以及多图归档与复用。桌面／手机浏览器验证真实上传、排序删除、图片切换、中文补生成和无横向溢出；不等同 NAS 客户端文件选择或系统分享验收。

真实订阅模型样本：使用专用 Antigravity 的 gemini-3.8-flash-medium（不使用 G1 credits），从一张合成移动界面完成两份英文文档、中文 DNA 和独立评分。Overview 为 113 词，英文文档未发现汉字，4 项中文特征与英文对应；图片中的推断和未观测行为有说明。模型给分 95，单图一致性维度排除；报告保留示例文案与原图存在差异的提示。这是单个真实模型样本，不保证所有输入质量相同；最后补充字体推断中文展示的兼容字段未额外调用模型。样本与页面截图在 `output/acceptance-016`、`output/playwright`，均不进入安装镜像。

amd64 独立容器已验证设置／资料库 API、上传 202、正式资料库只读、暂存可写、图片自动归档；没有账号的容器停在 WAITING_AUTH，并保留 0 分参考分。最终包沿用仅保留运行依赖、分离 Chromium／模型 CLI 的瘦身构建，不自动安装 NAS。

最终镜像补充核对：generation、service、presentation-job、image-import、render 五个核心源文件的 SHA-256 与最终 worker 镜像一致；最终 web 镜像的设置／资料库 API 返回 200，图片上传返回 202，library 只读与 imports 可写均通过实际文件操作验证。测试容器、卷和独立数据库已清理。

最终 0.1.6 LPK：573,521,920 字节（546.95 MiB），5 个 Linux amd64 镜像和 32 个唯一层的压缩摘要、解压摘要与大小全部校验通过。SHA-256：`29a8faf4ecdd8c0dc944e6a134a382d9b8ff710a5c1e54e2922b41d560b56294`。较 0.1.5 增加约 7.47 MiB，主要来自 web 图片解码／缩图运行依赖；继续排除开发依赖、采集样本、账号和缓存。未安装 NAS。
